    ...
    <div className={cn("w-full h-full", className)}>
      <ResponsiveContainer width="100%" height={height}>
        <AreaChart data={data} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
          ...
          {showXAxis && <XAxis dataKey={x} stroke="#888888" fontSize={12} tickLine={false} axisLine={false} />}
          {showYAxis && (
            <YAxis stroke="#888888" fontSize={12} tickLine={false} axisLine={false} tickFormatter={yFormatter} />
          )}
          {showTooltip && (
            <Tooltip
                  ...
                  return (
                    <div className="rounded-lg border border-border bg-card p-2 shadow-sm">
                      <div className="flex flex-col">
                        <span className="text-xs text-gray-400">{payload[0].payload[x]}</span>
                        <span className="font-bold text-teal">{yFormatter(payload[0].value as number)}</span>
                      </div>
                    </div>
                  )
                }
          ...
          <Area
            ...
            fill="url(#gradient)"
            dot={{ r: 0 }}
            activeDot={{ r: 6, fill: strokeColor, strokeWidth: 2, stroke: "white" }}
          />
        </AreaChart>
      </ResponsiveContainer>
    </div>
...
export function MiniChart({
  ...
  const gradientId = `miniGradient-${Math.random().toString(36).substring(2, 9)}`
          ...
          <Area
            ...
            dot={(props) => {
              ...
              if (index === 0 || index === data.length - 1 || index === Math.floor(data.length / 2)) {
                return <circle cx={cx} cy={cy} r={2} fill={strokeColor} stroke="white" strokeWidth={1} />
              }
              return null
            }}
          />
